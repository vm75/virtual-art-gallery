...
Museum rules are persisted as one draft row and one published row. `/admin/museum` provides a structured editor for groups, supported metadata conditions, priority/order, preview, and draft/published status; it validates through the same schema before saving. `/admin/museum/publish` explicitly promotes only a valid, layout-safe draft. `GET /api/museum` evaluates only the published snapshot against visible artworks and returns its version plus deterministic layout; an absent published snapshot is a public 404/fallback.
