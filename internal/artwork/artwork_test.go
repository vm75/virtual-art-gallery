...
func TestCreateNormalizeSlugAndPublicVisibility(t *testing.T) {
	...
	in := Input{Name: "  Blue / Hour  ", Date: "2024-01-02", Tags: []string{"Sky", " sky ", "Night"}, Surface: " Canvas ", Medium: " Oil "}
	first, err := r.Create(context.Background(), in)
	if err != nil {
		t.Fatal(err)
	}
	if first.Slug != "blue-hour" || len(first.Tags) != 2 || first.Surface != "canvas" {
	...
	updated, err := r.Update(context.Background(), first.Slug, Input{Name: "Blue Hour Updated", Date: "2024-01-02", Surface: "canvas", Medium: "oil", Visible: true})
	if err != nil || updated.Slug != first.Slug || updated.Name != "Blue Hour Updated" {
